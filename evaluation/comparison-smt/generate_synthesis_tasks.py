...
def generate_sample(topo, protocol, problem_index):
    ...
    img, _ = draw_graph(graph)
    imageio.imwrite('network.png', img)
    print("check network.png")
    ...
    with open("{}/{}-n{}.graphml".format(dataset_dir, protocol, problem_index), "w") as f:
        for line in generate_graphml(graph):
            f.write(line)

    with open("{}/{}-n{}.spec.json".format(dataset_dir, protocol, problem_index), "w") as f:
        json.dump(spec, f)
