...
def batch_evaluate_classical_gat_models():
    ...
    model_pattern = "models/fault_localization-model-classicalGAT-epoch*.pt"
    ...
    print(f"✅ 找到 {len(model_files)} 个 Classical GAT 模型文件")
    ...
    for dataset_name, validation_dataset in validation_datasets.items():
        ...
        for model_file in model_files:
            ...
            try:
                ...
                results = eval_fd_detailed(
                    ...
                    NUM_FAULT_CLASSES,
                    step_writer,
                    prefix=f"Val_ClassicalGAT_{dataset_name}_Epoch{epoch}",
                    device=device
                )

                # 保存详细结果（关键：文件名格式要匹配）
                if dataset_name == 'orign':
                    filename = f"./results/val_classicalGAT_epoch{epoch}.npz"
                else:
                    filename = f"./results/val_classicalGAT_epoch{epoch}_{dataset_name}.npz"
        ...
        # 7. 保存该数据集的汇总结果
        summary_file = f"./results/summary_classicalGAT_{dataset_name}.npz"
        np.savez(
            summary_file,
