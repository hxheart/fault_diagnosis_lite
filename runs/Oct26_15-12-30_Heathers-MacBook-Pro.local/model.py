...
def batch_evaluate_models():
    ...
    # 1. 查找所有模型文件
    model_pattern = "models/FD-GATv2-epoch*.pt"
    ...
    for dataset_name, validation_dataset in validation_datasets.items():
        ...
        for model_file in model_files:
            ...
            try:
                ...
                results = eval_fd_detailed(
                    ...
                    NUM_FAULT_CLASSES,
                    step_writer,
                    prefix=f"Val_GATv2_{dataset_name}_Epoch{epoch}",
                    device=device
                )

                # 保存详细结果（关键：文件名格式要匹配）
                if dataset_name == 'orign':
                    filename = f"./results/val_GATv2_epoch{epoch}.npz"
                else:
                    filename = f"./results/val_GATv2_epoch{epoch}_{dataset_name}.npz"

                # 保存详细结果
                np.savez(
                    f"./results/val_GATv2_epoch{epoch}_{dataset_name}.npz",
        ...
        # 7. 保存汇总结果
        summary_file = f"./results/summary_GATv2_{dataset_name}.npz"
        np.savez(
            summary_file,
