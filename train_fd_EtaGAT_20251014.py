...
class Model_fd_EtaGAT(torch.nn.Module): # maybe compare with the original "class Model()" in "synthesis task" to understand this code
...
if __name__ == '__main__':
    ...
    model = Model_fd_EtaGAT(features, HIDDEN_DIM, NUM_EDGE_TYPES, excluded_feature_indices).to(device)
    ...
    for epoch in tqdm(range(args.epochs), leave=False):
        ...
        # np.save('visual_loss.npy', np.array(visual_loss))
        # np.save('visual_accuracy.npy', np.array(visual_accuracy))
    ...
    ######################### run the evaluation code in another script ############################
        ...
        if epoch % 20 == 0 or epoch < 20 or epoch == NUM_TRAINING_EPOCH-1:
            ...
            # 保存详细结果到文件
            np.savez(f"./results/val_EtaGAT_epoch{epoch}.npz",
            ...
            torch.save([model.state_dict(), HIDDEN_DIM, NUM_EDGE_TYPES, excluded_feature_indices], f"models/{uid}-EtaGAT-epoch{epoch}.pt")

        writer.flush()

    # uid = os.environ.get("EXP_ID", "fault_localization")
    # os.makedirs("models", exist_ok=True)
    # torch.save([model.state_dict(), HIDDEN_DIM, NUM_EDGE_TYPES, excluded_feature_indices], f"models/{uid}-EtaGAT-final.pt")
